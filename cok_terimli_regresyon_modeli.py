...
# Kullanıcıdan en yüksek dereceyi alıyoruz
max_degree = int(input("Lütfen en yüksek dereceyi girin (örneğin, 3): "))

# Çok terimli terimler eklemek (dinamik şekilde)
X_extended = np.column_stack([X**i for i in range(1, max_degree+1)])  # 1'den max_degree'ye kadar olan tüm terimleri ekliyoruz
...
x_karamatrixyapma = np.column_stack((birlerdizisi, X_extended))  # Birler dizisi ve genişletilmiş X'i yana yana getiririz
...
print(f"y = {betalar[0]} ", end="")  # İlk β0 terimi
for i, beta in enumerate(betalar[1:], 1):  # Diğer β terimleri
    print(f"+ {beta}*X^{i} ", end="")  # Dereceyi belirterek çıktıyı yazdırıyoruz
print()
